...
if ( $this->ps["type"] == "pro" ){
	...
	// check session for referrer information
	if(!empty($_SESSION['__referrer'])):
		
		# save referrer log and rewards
		$referrer_parts = explode('__', $_SESSION['__referrer']);
		
		$track_id = end($referrer_parts);
		
		$referrer_username = str_replace('__' . $track_id, '', $_SESSION['__referrer']);
		
		$referrer_id = $loader->user->username_exists($referrer_username);
		
		if($referrer_id && $referrer_id != $loader->visitor->user()->ID):
		
			if ($loader->pay->pay_shared_song_commissions($referrer_id, $loader->visitor->user()->ID, $track_id)) unset($_SESSION['__referrer']);
