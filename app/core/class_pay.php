...
class pay {
	...
	public function get_shared_song_earnings( $user_id=null ){

		$user_id = $user_id ? $user_id : $this->loader->visitor->user()->ID;
		$shared_song_earnings = [];
		$shared_song_sql = "SELECT ustp.user_id, u.username AS sharer_username, u.name AS share_name, u.email AS sharer_email, ustp.buyer_id, ".
							"p.username AS purchaser_username, p.name AS purchaser_name, p.email AS purchaser_email, ustp.track_id, mt.title AS track_title, mt.artist_id, mt.artist_name, mt.hash AS track_hash, ustp.date_added, ustp.amount,
  ustp.commision_earned ".
							"FROM _users u INNER JOIN _user_shared_tracks_purchased ustp ON u.ID = ustp.user_id ".
							"INNER JOIN _users p ON ustp.buyer_id = p.ID ".
							"INNER JOIN _m_tracks mt ON ustp.track_id = mt.ID where ustp.user_id = '{$user_id}'";
					
		$get_shared_song_earnings = $this->db->query($shared_song_sql);
										
		if ( empty($get_shared_song_earnings) ) return $shared_song_earnings;
		while( $shared_song_earning = $get_shared_song_earnings->fetch_assoc() ){
			$shared_song_earnings[] = $shared_song_earning;
		}
		return $shared_song_earnings;

	}


	public function pay_shared_song_commissions($referrer_id, $buyer_id, $track_id, $commission_level = 'lvl_one'){
				
		# check if the referrer purchased the song initially
		$referrer_purchase_song = $this->loader->db->_select(array(
		...
		if(empty($referrer_purchase_song)) return false;
				
		$song = $this->loader->track->select(["ID"=>$track_id]);
		...
		switch($commission_level):
			...
			case 'lvl_one':
				$commission_to_pay = $song["price"] * 0.05;
				$next_level = 'lvl_two';
				break;
				
			case 'lvl_two':
				$commission_to_pay = $song["price"] * 0.03;
				$next_level = 'lvl_three';
				break;
				
			case 'lvl_three':
				$commission_to_pay = $song["price"] * 0.01;
				break;
		...
		$set = [
				[ "amount", $song["price"]],
				[ "buyer_id", $buyer_id ],
				[ "commision_earned", $commission_to_pay ],
				[ "track_id", $track_id ],
				[ "user_id", $referrer_id ],
		...
		$create = $this->db->_insert([
			"table" => "_user_shared_tracks_purchased",
			"set"   => $set
		]);
		
		if($create) $this->add_funds( $referrer_id, $commission_to_pay);
		
		# check if the referrer has a reference
		$reference = $this->loader->db->_select(array(
			"table" => "_user_shared_tracks_purchased",
			"where" => array(
				[ "buyer_id", "=", $referrer_id ],				
				[ "track_id", "=", $track_id ],
				[ "commission_level", "=", "lvl_one" ]
			),
			"limit" => 1
		));
		
		if(!empty($reference) && !empty($next_level)): 
		
			$reference = reset( $reference );
						
			$create = $this->pay_shared_song_commissions($reference['user_id'], $buyer_id, $track_id, $next_level);
